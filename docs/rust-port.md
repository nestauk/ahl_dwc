...
| C/C++ formatting and static analysis are deliberately non-blocking                                                          | `.github/workflows/format.yml:28-38` — the `cpp` job carries `continue-on-error: true`; the header explains it stays advisory "until a dedicated clang-format pass lands", because the ported `bw` sources are not Google-formatted and are meant to stay close to upstream                                                                                                                                                       |
...
3. **A checked-in `.clang-format` plus a repo-wide pass**, then drop `continue-on-error` from the `cpp`
   job (`format.yml:30`). This is blocked only by the "identical to upstream" policy in README's "Implementation details" table, and
   that policy can be satisfied instead by pinning the exact upstream commit of each vendored file in
   the README and diffing against it in CI. **1–2 days.**

- Effort: **4–7 days total.** Risk: **low.** Each item is independently shippable and independently
  revertible.
