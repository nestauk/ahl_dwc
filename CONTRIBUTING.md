...
| `format.yml` job `cpp` — `clang-format --style=Google` and `cppcheck`                                                          | No (`continue-on-error: true`) | The ported `bw` sources are not Google-formatted, so this is advisory until a dedicated clang-format pass lands. It reports green even when it finds things — **open the log** if you touched C++ |
...
Reformatting those files — including a future repo-wide `clang-format` pass — counts as changing them. Exclude them, or accept that the "identical to upstream" claim in the README and in `docs/adr/` has to be rewritten.
